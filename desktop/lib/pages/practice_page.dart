...
class _PracticePageState extends State<PracticePage> {
  ...
  Future<void> _loadBanks() async {
    try {
      ...
      setState(() {
        _banks = banks;
        _bankMap = {for (var b in banks) b.id: b};
      });
    } catch (_) {}
  }
        ...
        case PracticeModeType.bank:
          switch (_subMode) {
            case SubMode.sequential:
              data = await ApiService.sequentialQas(limit: 10, bankId: _bankId);
            case SubMode.random:
              data = await ApiService.randomQas(limit: 10, bankId: _bankId);
            case SubMode.wrong:
              data = await ApiService.wrongQas(limit: 10, bankId: _bankId, minWrong: _minWrong);
          }
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                if (qa.bankId != null)
                                  Chip(
                                    label: Text(_bankMap[qa.bankId]?.name ?? '', style: const TextStyle(fontSize: 11)),
