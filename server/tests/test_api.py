...
def test_create_qa():
    ...
    assert 'category_id' in data, "缺少 category_id"
    ...
    assert data['category_id'] == bank1["id"], f"category_id 值不对"
    assert set(data["tag_id"]) == {tag1["id"], tag2["id"]}, f"tag_id 值不对: {data['tag_id']}"
    qa1 = data


test("创建题目 - 验证全部字段 question/answer/image_url/total/right/wrong/random_int/category_id/tag_id", test_create_qa)
...
def test_create_qa_minimal():
    ...
    assert data['category_id'] is None, f"category_id 应为 None"
...
def test_create_qa_invalid_bank():
    resp = requests.post(f"{BASE}/api/qas", json={
        "question": "test",
        "answer": ["a"],
        'category_id': "nonexistent_bank_id",
    })
    assert resp.status_code == 500 or "不存在" in resp.text


test("创建题目 - 不存在的 category_id 应报错", test_create_qa_invalid_bank)
...
def test_page_qa_by_bank():
    resp = requests.get(f"{BASE}/api/qas", params={'category_id': bank1["id"]})
    assert resp.status_code == 200
    data = resp.json()
    assert data["total"] >= 1
    for item in data["items"]:
        assert item['category_id'] == bank1["id"], f"category_id 筛选不正确"


test("按分类筛选题目 - 验证 category_id", test_page_qa_by_bank)
...
test("按分类随机获取题目 - 验证 category_id", test_random_qa_by_bank)
